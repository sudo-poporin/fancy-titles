...
| `onPhaseChange` | Todos | Se ejecuta al cambiar de fase de animación |

### Fases de Animación

El enum `AnimationPhase` define las siguientes fases:

| Fase | Descripción |
|------|-------------|
| `idle` | Estado inicial antes de iniciar |
| `slideIn` | Elementos entrando en pantalla |
| `main` | Fase principal de la animación |
| `slideOut` | Elementos saliendo de pantalla |
| `completed` | Animación finalizada |
...
Todos los widgets soportan controllers externos para control programático de las
animaciones. Esto permite pausar, reanudar, saltar al final o reiniciar la
animación desde código.

### Controllers Disponibles

| Controller | Widget | Descripción |
|------------|--------|-------------|
| `SonicManiaSplashController` | `SonicManiaSplash` | Control de animación Sonic Mania |
| `Persona5TitleController` | `Persona5Title` | Control de animación Persona 5 |
| `EvangelionTitleController` | `EvangelionTitle` | Control de animación Evangelion |
| `MarioMakerTitleController` | `MarioMakerTitle` | Control de animación Mario Maker |

### Métodos del Controller

| Método | Descripción |
|--------|-------------|
| `pause()` | Pausa la animación en el frame actual |
| `resume()` | Reanuda la animación desde donde fue pausada |
| `skipToEnd()` | Salta directamente al final de la animación |
...
|-----------|------|-------------|
| `currentPhase` | `AnimationPhase` | Fase actual de la animación |
| `isPaused` | `bool` | Si la animación está pausada |
| `isCompleted` | `bool` | Si la animación ha finalizado |
...
class _MySplashScreenState extends State<MySplashScreen> {
  final _controller = SonicManiaSplashController();
...
_controller.addListener(() {
  print('Fase actual: ${_controller.currentPhase}');
  print('Pausado: ${_controller.isPaused}');
});
```
...
Cada widget soporta personalización completa de colores a través de clases
Theme dedicadas. Podés cambiar todos los colores de las animaciones para
adaptarlos a tu marca o diseño.

### Themes Disponibles

| Theme | Widget | Colores Personalizables |
|-------|--------|------------------------|
| `SonicManiaTheme` | `SonicManiaSplash` | Barras, cortinas, fondos de texto |
| `Persona5Theme` | `Persona5Title` | Círculos, texto, trazo |
| `EvangelionTheme` | `EvangelionTitle` | Fondo, texto, sombras, cortinas |
| `MarioMakerTheme` | `MarioMakerTitle` | Fondo, círculo, título |
...
SonicManiaSplash(
  ...
  theme: SonicManiaTheme(
    // Colores de barras diagonales
    barColors: SonicManiaBarColors(
      red: Colors.purple,
      orange: Colors.pink,
      blue: Colors.indigo,
      green: Colors.teal,
    ...
    // Colores de cortinas
    curtainColors: SonicManiaCurtainColors(
      blue: Colors.deepPurple,
      orange: Colors.amber,
      amber: Colors.orange,
    ...
    // Colores de barras de texto
    textBarColors: SonicManiaTextBarColors(
      blackBackground: Colors.grey[850]!,
      whiteBackground: Colors.grey[100]!,
      blackText: Colors.grey[900]!,
...
Persona5Title(
  ...
  theme: Persona5Theme(
    // Colores de círculos del fondo
    circleColors: Persona5CircleColors(
      red: Color(0xFFFF1518),
      black: Colors.black,
    ),
    // Colores del texto
    textColors: Persona5TextColors(
      fill: Colors.white,
      stroke: Color(0xFFFF1518),
    ),
  ),
)
```
...
EvangelionTitle(
  ...
  theme: EvangelionTheme(
    ...
    // Colores de texto
    textColors: EvangelionTextColors(
      primary: Color(0xFFF1EEFF),
      shadow: Colors.black,
    ),
    // Colores de cortinas
    curtainColors: EvangelionCurtainColors(
      first: Colors.black,
      second: Colors.black87,
      third: Colors.black54,
      fourth: Colors.black45,
      fifth: Colors.black38,
    ),
  ),
)
```
...
MarioMakerTitle(
  ...
  theme: MarioMakerTheme(
    // Colores de fondo
    backgroundColors: MarioMakerBackgroundColors(
      initial: Colors.black,
      expanded: Color(0xFFFFC800),  // Amarillo Mario
    ),
    // Color del círculo
...
### Usando Temas Globalmente

Podés envolver tu app con `FancyTitlesTheme` para aplicar temas a todos los
widgets descendientes:

```dart
FancyTitlesTheme(
  sonicMania: SonicManiaTheme(/* ... */),
  persona5: Persona5Theme(/* ... */),
  evangelion: EvangelionTheme(/* ... */),
  marioMaker: MarioMakerTheme(/* ... */),
  child: MaterialApp(
    home: MyHomePage(),
  ),
)
...
- Más pantallas de inicio personalizadas (Zelda, Pokemon, etc.)
- Soporte para animaciones de sonido sincronizadas
- Widget contenedor con navegación automática
- Benchmarks automatizados en CI/CD
